...
import bimoku.realtimecrawler.control.ControlAmazonPromotion;
...
public class CrawlerAmazonPromotion {
	...
	public static void extract(String configPath, Parser parser) throws MalformedURLException {
		...
		System.out.println(PropertyUtil.readProperty(PropertyUtil.starturl_promotion));
		...
		while (!firstUrlQueue.isQueueEmpty()) {
			...
			ControlAmazonPromotion controlamazon = new ControlAmazonPromotion(firstUrl, parser, configPath);
	...
	public static void crawler() throws BeansException, MalformedURLException {
		extract("amazonConfig.properties",
				(ParserAmazon) getContext().getBean("parserAmazon"));
	}
