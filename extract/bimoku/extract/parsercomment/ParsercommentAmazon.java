...
public class ParsercommentAmazon {

	public static void main(String[] args) throws InterruptedException {
		SetIpProxy setIpProxy = new SetIpProxy();
		setIpProxy.setIpProxy();
		PropertyUtil.getProperty("amazonConfig.properties");
		List<Comment> comments = parsercomment("9787540463540","http://www.amazon.cn/product-reviews/B00ARB0HBO/ref=cm_cr_dp_see_all_btm?ie=UTF8&showViewpoints=1&sortBy=bySubmissionDateDescending");
		//TODO ..........INSERT TO DB
	}
	...
	private static List<Comment> Todetail(String isbn, ArrayList commentlist) {
		...
		while (it1.hasNext()) {
			...
			String com_content = doc.ownText();
			com_content = com_content.replaceAll("这条评论对您有用吗？ 是的 不", "");
		...
		//	comment.setCom_avatar(com_avatar.attr("src").substring(0,com_avatar.attr("src").length()>150?150:com_avatar.attr("src").length()));
			...
			comment.setCom_zhandian("Amazon");
			// TODO
			comms.add(comment);		
		}
		return comms;
