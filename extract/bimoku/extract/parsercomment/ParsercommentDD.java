...
public class ParsercommentDD {

	public static void main(String[] args) throws InterruptedException, UnknownHostException {
		SetIpProxy setIpProxy = new SetIpProxy();
		setIpProxy.setIpProxy();
		PropertyUtil.getProperty("ddConfig.properties");
		List<Comment> comments = parsercomment("9787540463540","http://comm.dangdang.com/comment/review_list.php?pid=23317693");
		//TODO ..........INSERT TO DB
	}
	...
	public static List<Comment> parsercomment(String isbn, String commenturl)
		...
		while (count > 0) {
			String commenthtml = "";
			commenthtml = httpcon.getHtml(com_nextPage);
			...
			if(com_nextPage==null||" ".equals(com_nextPage)||!com_nextPage.matches("http://comm.dangdang.com/reviewlist")){
				break;
			}
			System.out.println(com_nextPage+"_____________");
			count--;
		}
	...
	private static List<Comment> Todetail(String isbn, ArrayList commentlist) {
		...
		while (it1.hasNext()) {
			...
			Element create_at = doc.select(
					PropertyUtil.readProperty(PropertyUtil.create_at)).last();
			Comment comment = new Comment();
